...
document.addEventListener('DOMContentLoaded', function() {
    ...
    waitUntil(() => document.getElementById('locations-menu')).then((locationsMenu) => {
        if (locationsMenu) {
    ...
    waitUntil(() => document.querySelector('.hero')).then((heroSection) => {
        if (heroSection) {
    ...
    waitUntil(() => document.querySelector('#form-prenotazione h3')).then((formHeading) => {
        if (formHeading) {
